...
void Flashable::setColorScheme(QColor letterOn, QColor labelOn,
...
void Flashable::labelOn(int row, int column)
{
...
void Flashable::labelOff(int row, int column)
{
    int index = column + row*width_;
    QLabel* label = vLabels_.at(index);
    label->setPalette(inactiveLabelPalette);
...
int Flashable::getNumberOfLabels() const
{
    return vLabels_.size();
}

bool Flashable::oneByOneSearch()
{
    static int index = 0;
...
QVector<QString> Flashable::getActiveLabelsContent() const
{
